...
massa_wallet_address=$(./massa/massa-client --pwd $massa_pass wallet_info | grep Address | awk '{ print $2 }')
while true
do
        balance=$(./massa/massa-client --pwd $massa_pass wallet_info | grep "Final balance" | awk '{ print $3 }')
        int_balance=${balance%%.*}
        if [ $int_balance -gt "99" ]; then
                echo "More than 99"
                resp=$(./massa/massa-client --pwd $massa_pass buy_rolls $massa_wallet_address $(($int_balance/100)) 0)
                echo $resp
